...
import { fixtureIdentityIsValid } from "./fixture-identity";
...
export function Dashboard() {
  ...
  useEffect(() => {
    if (!publicConfig.apiUrl) return;
    const controller = new AbortController();
    void fetch(`${publicConfig.apiUrl}/api/v1/fixtures`, { signal: controller.signal, headers: { Accept: "application/json" } })
      .then((response) => response.ok ? response.json() : Promise.reject(new Error("Fixture source unavailable")))
      .then((payload: { fixtures?: { id?: string; fixture_identity?: { arawee_fixture_id?: string } }[] }) => {
        const fallbackById = new Map(upcomingFixtures.map((item) => [item.id, item]));
        const verified = (payload.fixtures || []).flatMap((item) => {
          const fallback = item.id ? fallbackById.get(item.id) : undefined;
          return fallback && item.fixture_identity && fixtureIdentityIsValid({ ...fallback.identity, arawee_fixture_id: item.fixture_identity.arawee_fixture_id || "" }, fallback.id) ? [fallback] : [];
        });
        if (verified.length) {
          setFixtures(verified);
          setFixture((current) => verified.find((item) => item.id === current.id) || verified[0]);
        }
      })
      .catch(() => undefined);
  ...
  return <main className="terminal-shell">
    ...
    <section className="terminal-intro"><div><p className="eyebrow">PRE-MATCH DECISION DESK</p><h1>Find the price.<br /><em>Keep the proof.</em></h1><p>Market-aware football probabilities, uncertainty and evidence — designed to abstain before pretending to know.</p></div><div className="intro-stamp"><span>UPCOMING FIXTURES</span><strong>{fixtures.length} matches on the board</strong><small>All listed times are shown in Central European Summer Time.</small></div></section>
    ...
    <section className="integration-settings" aria-labelledby="integration-title"><div><p className="eyebrow">SETTINGS · INTEGRATIONS</p><h2 id="integration-title">Connected services.</h2><p>Secrets and provider tokens stay on the server. This public build only shows integration status.</p></div><div className="integration-list"><span>Football data · Sportmonks</span><span>Odds data · The Odds API</span><span>AI analyst · Gemini</span></div><SportzfyIntegration /></section>
