...
type LiveUpdate = { fixture_id: string | number; name: string; starting_at: string | null; score_count: number; event_count: number };
...
export function LiveFeed() {
  ...
  return <section className="live-feed" aria-labelledby="live-feed-title">
    <div className="live-feed-heading"><div><p className="eyebrow">PROTECTED LIVE FOOTBALL FEED</p><h2 id="live-feed-title">Latest match changes.</h2></div><div><span className={`live-status ${status.toLowerCase()}`}>{status}</span><button type="button" onClick={() => void refresh()} disabled={status === "LOADING"}>{status === "LOADING" ? "Checking…" : "Refresh"}</button></div></div>
    {status === "LIVE" && feed ? <><p className="live-feed-note">Source: {feed.provider ?? "protected provider"} · Checked {checkedTime(feed.checked_at)} · Shared cache: {feed.cache_seconds}s. {feed.notice}</p>{feed.updates.length ? <ul className="live-update-list">{feed.updates.slice(0, 8).map((update) => <li key={update.fixture_id}><strong>{update.name}</strong><span>{update.score_count} score update{update.score_count === 1 ? "" : "s"} · {update.event_count} event{update.event_count === 1 ? "" : "s"}</span></li>)}</ul> : <div className="live-empty">No fixture changed in the latest provider update window. This is normal between events.</div>}</> : null}
