...
const starterQuestions = ["Why WATCH?", "Home team at 2.20, UGX 10,000", "What is happening live?", "What are the best bets this week?"];
...
function localReply(question: string, fixtureName: string): AssistantReply {
  ...
  if (["live", "happening", "score", "xg", "lineup", "injury", "news today"].some((term) => normalised.includes(term))) {
    return { mode: "DEMO", data_status: "LIVE_UNAVAILABLE", decision: "WATCH", reasons: ["WATCH_DATA", "WATCH_LINEUP", "WATCH_PRICE"], answer: "The decision desk has no authorised score, lineup, injury, odds or news feed for this scheduled fixture, so it will not invent an update.", facts: [`Fixture context: ${fixtureName}.`, "Data status: schedule only; no live match model is active.", "Connect authorised providers before relying on live match information."], suggestions: starterQuestions, disclaimer: "Built-in deterministic demo assistant; missing live facts are never guessed." };
  }
  if (["why", "explain", "watch", "no bet", "qualified", "decision"].some((term) => normalised.includes(term))) {
    return { mode: "DEMO", data_status: "DEMO_ONLY", decision: "WATCH", reasons: ["WATCH_DATA", "WATCH_LINEUP", "WATCH_PRICE"], answer: "The current decision is WATCH. It is waiting for stronger data, confirmed lineups and a current manually entered price before anything can qualify.", facts: [`Fixture context: ${fixtureName}.`, "The fixture list is scheduled data, not a live recommendation.", "No result or betting outcome is promised."], suggestions: starterQuestions, disclaimer: "Built-in deterministic demo assistant; it does not use a cloud AI service." };
  }
  if (["best bet", "best bets", "week", "strongest", "recommend"].some((term) => normalised.includes(term))) {
    return { mode: "DEMO", data_status: "DEMO_ONLY", decision: "WATCH", reasons: ["WATCH_DATA", "WATCH_LINEUP", "WATCH_PRICE"], answer: "There are no qualified opportunities. The system is intentionally abstaining until data, lineup and price gates pass.", facts: ["Qualified opportunities: 0.", `Selected fixture: ${fixtureName}.`, "No fictional ranking is substituted for missing live data."], suggestions: starterQuestions, disclaimer: "Built-in deterministic demo assistant; no cloud AI service is involved." };
  }
...
export function AnalystAssistant({ fixture }: { fixture: Fixture }) {
  ...
  async function ask(nextQuestion: string) {
    ...
    setQuestion(trimmed);
    setLoading(true);
    setReply(localReply(trimmed, fixtureName));
    setLoading(false);
  }
