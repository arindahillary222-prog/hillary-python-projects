...
async function handleAlerts(request, env) {
  ...
  const emailConfigured = Boolean(env.RESEND_API_KEY && env.ALERT_FROM_EMAIL);
  const confirmationSent = emailConfigured ? await sendConfirmation(alert, env).catch(() => false) : false;
  ...
  return json(202, {
    ...
    message: emailConfigured
      ...
      : "Alert accepted locally. Configure RESEND_API_KEY and ALERT_FROM_EMAIL in Cloudflare Pages for email delivery."
  });
}
...
async function sendConfirmation(alert, env) {
